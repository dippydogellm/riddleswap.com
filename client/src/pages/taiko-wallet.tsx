...
export default function TaikoWallet() {
  ...
  const taikoAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('taiko', taikoAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('taiko');
    if (!auth) {
  ...
  if (!isAuthenticated || !taikoAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.taiko}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Taiko wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/taiko/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/taiko/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=taiko') },
