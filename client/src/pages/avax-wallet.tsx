...
export default function AvaxWallet() {
  ...
  const avaxAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('avalanche', avaxAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('avalanche');
    if (!auth) {
  ...
  if (!isAuthenticated || !avaxAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.avalanche}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Avalanche wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/avalanche/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/avalanche/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=avalanche') },
