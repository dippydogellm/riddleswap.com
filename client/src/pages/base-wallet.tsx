...
export default function BaseWallet() {
  ...
  const baseAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('base', baseAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('base');
    if (!auth) {
  ...
  if (!isAuthenticated || !baseAddress) {
    return (
      <WalletUpgradeTemplate
        ...
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Base wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/base/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/base/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=base') },
