...
function CollectionCard({ collection }: { collection: NFTCollection }) {
  ...
  return (
    <Link href={`/nft-collection/eth/${collection.address}`}>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                {collection.floor_price !== undefined && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-300">
                      {collection.floor_price.toFixed(4)} ETH
                    </span>
                  </div>
                )}
                ...
                {collection.volume_24h !== undefined && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-300">
                      {collection.volume_24h.toFixed(2)} ETH/24h
                    </span>
                  </div>
                )}
...
export default function EthMarketplace() {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'verified' | 'trending' | 'new'>('all');
  
  // Fetch ETH collections
  const { data: collectionsData, isLoading } = useQuery({
    queryKey: ['/api/nftscan/eth/collections'],
    queryFn: async () => {
      const response = await fetch('/api/nftscan/eth/collections');
      if (!response.ok) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400">
                Explore and trade NFTs on Ethereum mainnet
              </p>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">
                    {collections.reduce((sum, c) => sum + (c.volume_24h || 0), 0).toFixed(2)} ETH
                  </p>
                </div>
