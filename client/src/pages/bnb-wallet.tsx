...
export default function BnbWallet() {
  ...
  const bnbAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('bsc', bnbAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('bsc');
    if (!auth) {
  ...
  if (!isAuthenticated || !bnbAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.bsc}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your BNB wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/bsc/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/bsc/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=bsc') },
      ...
      >
        ...
        {activeTab === 0 && (
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2 }}>BEP-20 Tokens ({tokens.length})</Typography>
              {tokens.length === 0 ? (
