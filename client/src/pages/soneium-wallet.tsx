...
export default function SoneiumWallet() {
  ...
  const soneiumAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('soneium', soneiumAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('soneium');
    if (!auth) {
  ...
  if (!isAuthenticated || !soneiumAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.soneium}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Soneium wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/soneium/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/soneium/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=soneium') },
