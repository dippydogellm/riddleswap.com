...
export default function OptimismWallet() {
  ...
  const optimismAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('optimism', optimismAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('optimism');
    if (!auth) {
  ...
  if (!isAuthenticated || !optimismAddress) {
    return (
      <WalletUpgradeTemplate
        ...
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Optimism wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/optimism/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/optimism/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=optimism') },
