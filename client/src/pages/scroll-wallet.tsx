...
export default function ScrollWallet() {
  ...
  const scrollAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('scroll', scrollAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('scroll');
    if (!auth) {
  ...
  if (!isAuthenticated || !scrollAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.scroll}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Scroll wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/scroll/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/scroll/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=scroll') },
