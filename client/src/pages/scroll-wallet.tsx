...
import '@/styles/scroll-wallet.css';
...
export default function ScrollWallet() {
  ...
  console.log('🔍 [SCROLL PAGE] Debugging:', {
    authLoading,
    isAuthenticated,
    walletAddress,
  ...
  // Fetch Scroll balance using direct endpoint
  const { data: balanceData, isLoading, refetch, error } = useQuery({
    queryKey: ['/api/wallets/scroll/balance', walletAddress],
    queryFn: async () => {
      console.log('🚀 [SCROLL QUERY] Starting balance fetch for:', walletAddress);
      const response = await fetch(`/api/wallets/scroll/balance/${walletAddress}`, {
        headers: { 'Authorization': `Bearer ${authData?.sessionToken}` }
      });
      console.log('📥 [SCROLL QUERY] Response status:', response.status);
      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ [SCROLL QUERY] Error response:', errorText);
        throw new Error(`Failed to fetch balance: ${response.status}`);
      }
      const data = await response.json() as any;
      console.log('✅ [SCROLL QUERY] Balance data received:', data);
      return data;
    },
  ...
  console.log('📊 [SCROLL PAGE] Query state:', {
    isLoading,
  ...
  const handleCopyAddress = () => {
    if (balanceData?.address) {
      ...
      toast({
        ...
        description: "Scroll address copied to clipboard"
      });
  ...
  const handleRefresh = () => {
    ...
    toast({
      title: "Refreshing",
      description: "Updating Scroll wallet data..."
    });
  };
      ...
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardContent className="p-6 text-center">
            <h2 className="text-xl font-bold mb-4">Scroll Wallet Error</h2>
  ...
  return (
    <div className="scroll-wallet-container min-h-screen bg-gradient-to-b from-orange-50 to-orange-100 dark:from-gray-900 dark:to-black py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">zkEVM Layer 2 Network</p>
            </div>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">Transactions will appear here once you start using your Scroll wallet</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">You'll be able to send ETH on Scroll network</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
