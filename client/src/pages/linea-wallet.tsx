...
import '@/styles/linea-wallet.css';
...
export default function LineaWallet() {
  ...
  console.log('🔍 [LINEA PAGE] Debugging:', {
    authLoading,
    isAuthenticated,
    walletAddress,
    ...
    walletAddressesKeys: walletAddresses ? Object.keys(walletAddresses) : []
  });

  // Fetch Linea balance using direct endpoint
  const { data: balanceData, isLoading, refetch, error } = useQuery({
    queryKey: ['/api/wallets/linea/balance', walletAddress],
    queryFn: async () => {
      console.log('🚀 [LINEA QUERY] Starting balance fetch for:', walletAddress);
      const response = await fetch(`/api/wallets/linea/balance/${walletAddress}`, {
        headers: { 'Authorization': `Bearer ${authData?.sessionToken}` }
      });
      console.log('📥 [LINEA QUERY] Response status:', response.status);
      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ [LINEA QUERY] Error response:', errorText);
        throw new Error(`Failed to fetch balance: ${response.status}`);
      }
      const data = await response.json() as any;
      console.log('✅ [LINEA QUERY] Balance data received:', data);
      return data;
    },
  ...
  console.log('📊 [LINEA PAGE] Query state:', {
    isLoading,
  ...
  console.log('🎯 [LINEA PAGE] Render decision:', {
    willShowLoading: isLoading,
    willShowError: !balanceData && !isLoading,
    willShowContent: !!balanceData,
    authLoading,
    isAuthenticated
  });
  ...
  const handleCopyAddress = () => {
    if (balanceData?.address) {
      ...
      toast({
        ...
        description: "Linea address copied to clipboard"
      });
  ...
  const handleRefresh = () => {
    ...
    toast({
      title: "Refreshing",
      description: "Updating Linea wallet data..."
    });
  };
      ...
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardContent className="p-6 text-center">
            <h2 className="text-xl font-bold mb-4">Linea Wallet Error</h2>
  ...
  return (
    <div className="linea-wallet-container min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-black py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">ConsenSys Layer 2 Network</p>
            </div>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">Transactions will appear here once you start using your Linea wallet</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">You'll be able to send ETH on Linea network</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
