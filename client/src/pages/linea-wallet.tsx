...
export default function LineaWallet() {
  ...
  const lineaAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('linea', lineaAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('linea');
    if (!auth) {
  ...
  if (!isAuthenticated || !lineaAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.linea}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Linea wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/linea/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/linea/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=linea') },
