...
export default function UnichainWallet() {
  ...
  const unichainAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('unichain', unichainAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('unichain');
    if (!auth) {
  ...
  if (!isAuthenticated || !unichainAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.unichain}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Unichain wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.unichain}
        address={unichainAddress}
        balance={{ native: balance, usd: `$${parseFloat(balanceUsd).toFixed(2)}` }}
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/unichain/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/unichain/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=unichain') },
