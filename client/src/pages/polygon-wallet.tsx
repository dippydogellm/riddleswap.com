...
export default function PolygonWallet() {
  ...
  const polygonAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('polygon', polygonAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('polygon');
    if (!auth) {
  ...
  if (!isAuthenticated || !polygonAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.polygon}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Polygon wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/polygon/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/polygon/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=polygon') },
