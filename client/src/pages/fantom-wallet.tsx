...
export default function FantomWallet() {
  ...
  const fantomAddress = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('fantom', fantomAddress, {
    includeTokens: true,
  ...
  const confirmBurnDust = async () => {
    const auth = await getTransactionAuth('fantom');
    if (!auth) {
  ...
  if (!isAuthenticated || !fantomAddress) {
    return (
      <WalletUpgradeTemplate
        chainConfig={CHAIN_CONFIGS.fantom}
        address=""
        balance={{ native: '0', usd: '0' }}
        onRefresh={handleRefresh}
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your Fantom wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
  ...
  return (
    <>
      <WalletUpgradeTemplate
        ...
        onRefresh={handleRefresh}
        customActions={[
          { label: 'Send', icon: 'send', onClick: () => navigate('/fantom/send') },
          { label: 'Receive', icon: 'receive', onClick: () => navigate('/fantom/receive') },
          { label: 'Swap', icon: 'swap', onClick: () => navigate('/trade-v3?chain=fantom') },
