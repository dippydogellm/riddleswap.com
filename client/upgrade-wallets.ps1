# PowerShell script to upgrade all EVM wallet files to Material UI

$wallets = @(
    @{name="bnb"; chain="bnb"; displayName="BNB"; token="BEP-20"},
    @{name="base"; chain="base"; displayName="Base"; token="ERC-20"},
    @{name="avax"; chain="avax"; displayName="Avalanche"; token="ERC-20"},
...
foreach ($wallet in $wallets) {
    ...
    $token = $wallet.token
    $capName = (Get-Culture).TextInfo.ToTitleCase($name) + "Wallet"
...
"@
    ...
    $filePath = "src\pages\$name-wallet.tsx"
    Write-Host "Creating $filePath..."
    $content | Out-File -FilePath $filePath -Encoding utf8 -Force
}

Write-Host "`nAll wallet files upgraded successfully!" -ForegroundColor Green
