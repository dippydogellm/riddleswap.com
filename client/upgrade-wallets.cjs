...
const wallets = [
  { name: "bnb", chain: "bsc", displayName: "BNB", token: "BEP-20" },
  { name: "base", chain: "base", displayName: "Base", token: "ERC-20" },
  { name: "avax", chain: "avalanche", displayName: "Avalanche", token: "ERC-20" },
  { name: "polygon", chain: "polygon", displayName: "Polygon", token: "ERC-20" },
  { name: "arbitrum", chain: "arbitrum", displayName: "Arbitrum", token: "ERC-20" },
  { name: "optimism", chain: "optimism", displayName: "Optimism", token: "ERC-20" },
  { name: "fantom", chain: "fantom", displayName: "Fantom", token: "ERC-20" },
  { name: "zksync", chain: "zksync", displayName: "zkSync", token: "ERC-20" },
  { name: "linea", chain: "linea", displayName: "Linea", token: "ERC-20" },
  { name: "taiko", chain: "taiko", displayName: "Taiko", token: "ERC-20" },
  { name: "unichain", chain: "unichain", displayName: "Unichain", token: "ERC-20" },
  { name: "soneium", chain: "soneium", displayName: "Soneium", token: "ERC-20" },
  { name: "mantle", chain: "mantle", displayName: "Mantle", token: "ERC-20" },
  { name: "metis", chain: "metis", displayName: "Metis", token: "ERC-20" },
  { name: "scroll", chain: "scroll", displayName: "Scroll", token: "ERC-20" }
];

for (const wallet of wallets) {
  const { name, chain, displayName, token } = wallet;
  const capName = name.charAt(0).toUpperCase() + name.slice(1) + "Wallet";
...
export default function ${capName}() {
  ...
  const ${name}Address = authData?.walletAddresses?.eth || authWalletData?.ethAddress;
  
  const walletData = useOptimizedWalletData('${chain}', ${name}Address, {
    includeTokens: true,
  ...
  if (!isAuthenticated || !${name}Address) {
    return (
      <WalletUpgradeTemplate
        ...
        customActions={[]}
      >
        <Alert severity="info">Please log in to access your ${displayName} wallet</Alert>
      </WalletUpgradeTemplate>
    );
  }
    ...
    <>
      ...
      >
        ...
        {activeTab === 0 && (
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2 }}>${token} Tokens ({tokens.length})</Typography>
              {tokens.length === 0 ? (
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {tokens.map((token: any, i: number) => (
                    <Box key={i} sx={{ p: 2, border: '1px solid rgba(255,255,255,0.1)', borderRadius: 1 }}>
                      ...
                      <Typography variant="body2" color="text.secondary">Value: ${parseFloat(token.balanceUsd || '0').toFixed(2)}</Typography>
                    </Box>
                  ))}
                </Box>
              )}
...
`;
  ...
  const filePath = path.join(__dirname, 'src', 'pages', `${name}-wallet.tsx`);
  console.log(`Creating ${name}-wallet.tsx...`);
  fs.writeFileSync(filePath, content, 'utf8');
}

console.log('\nAll wallet files upgraded successfully!');
