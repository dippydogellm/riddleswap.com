...
router.post("/api/gaming/squadrons", requireAuthentication, async (req: AuthenticatedRequest, res: Response) => {
  try {
    console.log("🎯 [SQUADRON CREATE] Squadron creation request received");
    const userHandle = req.user?.handle || req.user?.userHandle;
    
    if (!userHandle) {
      console.error("❌ [SQUADRON CREATE] No user handle in request");
      return res.status(401).json({ error: "Not authenticated" });
    }

    console.log("👤 [SQUADRON CREATE] User:", userHandle);
    
    // Validate request body with Zod schema
    let validatedData;
    try {
      validatedData = createSquadronSchema.parse(req.body);
      console.log("✅ [SQUADRON CREATE] Request data validated");
    } catch (validationError: any) {
      console.error("❌ [SQUADRON CREATE] Validation failed:", validationError.errors);
    ...
    console.log("📝 [SQUADRON CREATE] Request data:", { 
      name, 
    ...
    if (!player) {
      console.log("👤 [SQUADRON] Auto-creating gaming player for:", userHandle);
      ...
      player = newPlayer;
      console.log("✅ [SQUADRON] Created gaming player:", player.id);
    }

    console.log("🆔 [SQUADRON CREATE] Will use auto-generated squadron ID from schema");
    ...
    try {
      ...
      console.log("✅ [SQUADRON CREATE] Squadron inserted successfully:", {
        id: insertedSquadron.id,
        name: insertedSquadron.name,
        player_id: insertedSquadron.player_id
      });

      console.log("👥 [SQUADRON CREATE] Adding", nfts.length, "NFT members...");
      ...
      console.log("✅ [SQUADRON CREATE] All squadron members added successfully:", {
        count: insertedMembers.length,
        members: insertedMembers.map(m => ({ id: m.id, nft_name: m.nft_name }))
      });

      console.log("🎉 [SQUADRON CREATE] Squadron creation complete:", insertedSquadron.id);
      ...
      console.error("❌ [SQUADRON CREATE] Database error:", {
    ...
    console.error("❌ [SQUADRON CREATE] Error creating squadron:", error);
    console.error("❌ [SQUADRON CREATE] Error stack:", error.stack);
...
router.get("/api/gaming/squadrons", requireAuthenticationReadOnly, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!userHandle) {
      console.error("❌ [GET SQUADRONS] No user handle in request");
      return res.status(401).json({ error: "Not authenticated" });
    }
...
router.post("/api/gaming/battles/create", requireAuthentication, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!userHandle) {
      console.error("❌ [BATTLE CREATE] No user handle in request");
      return res.status(401).json({ error: "Not authenticated" });
    }

    console.log("🎯 [BATTLE CREATE] Battle creation request received from:", userHandle);
    
    // Validate request body with Zod schema
    let validatedData;
    try {
      validatedData = createBattleSchema.parse(req.body);
      console.log("✅ [BATTLE CREATE] Request data validated");
    } catch (validationError: any) {
      console.error("❌ [BATTLE CREATE] Validation failed:", validationError.errors);
      return res.status(400).json({ 
        error: "Invalid request data", 
        details: validationError.errors 
      });
    ...
    console.log("💾 [BATTLE CREATE] Creating battle in database...");
  ...
  creator_player_id: player.id,
      ...
      console.log("✅ [BATTLE CREATE] Battle created successfully:", {
        id: newBattle.id,
